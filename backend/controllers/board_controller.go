...
func (c *BoardController) UpdateBoard (ctx *fiber.Ctx) error {
	...
	return utils.Success(ctx, "Board Berhasil di-update", board)
...
func (c *BoardController) RemoveBoardMembers(ctx *fiber.Ctx) error {
	...
	if err := c.service.RemoveMembers(publicID, userIDs); err != nil {
		return utils.BadRequest(ctx, "Gagal Me-remove Members", err.Error())
	}
...
func (c *BoardController) GetBoardDetail(ctx *fiber.Ctx) error {
	publicID := ctx.Params("id")
	...
	return utils.Success(ctx, "Berhasil Mengambil Detail Board", board)
}
...
func (c *BoardController) GetBoardMembers(ctx *fiber.Ctx) error {
	...
	if err != nil {
		return utils.NotFound(ctx, "Gagal mengambil data members", err.Error())
	}

	return utils.Success(ctx, "Berhasil mengambil members", members)
}